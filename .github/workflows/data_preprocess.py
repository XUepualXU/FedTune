...
def data_init(FL_params):
    ...
    kwargs = {'num_workers': 0, 'pin_memory': True} if FL_params.cuda_state else {}   # 如果当前电脑没有CUDA，则kwargs为{}
    ...
    if FL_params.data_name == 'mnist' or FL_params.data_name == 'cifar10':
        split_index = [int(trainset.__len__() / FL_params.N_total_client)] * FL_params.N_total_client
        client_dataset = torch.utils.data.random_split(trainset, split_index)

    if FL_params.data_name == 'adult' or FL_params.data_name == 'purchase':
        split_index = [int(trainset.__len__() / FL_params.N_total_client)] * (FL_params.N_total_client - 1)
        split_index.append(int(trainset.__len__() - int(trainset.__len__() / FL_params.N_total_client) * (
    ...
    # # 随机选择 data_len_need 个样本的索引
    # data_len_need = int(trainset.__len__() / FL_params.N_total_client) * FL_params.N_total_client
    # random_indices = random.sample(range(total_samples), data_len_need)
    # # 创建一个新的 TensorDataset 包含随机选择的样本
    # trainset_samples = [trainset[i] for i in random_indices]
    # trainset = TensorDataset(*trainset_samples)
    ...
    client_dataset = torch.utils.data.random_split(trainset, split_index)  # torch.utils.data.random_split(dataset, lengths) 将数据集按照lengths长度随即划分
